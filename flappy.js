...
window.onload = function () {
    ...
    //load images
    birdImg = new Image();
    birdImg.src = "./Italian_Mafia_Boss_alpha.png";
...
function start() {
    board = document.getElementById("board");
    blurz = document.getElementById("blur")
...
function topScore() {
    ...
    fetch('https://qr-code-api.oasisx.world/flappy-get-all-user', {
        ...
        .then(data => {
            ...
            topScore.innerHTML = `
            <p style="margin: 0;"> <span style="color: #ff4655">TOP SCORE #</span><span
                    style="color: #f2f2f2;"> ${data.scores[0].score} </span></p>
                <p style="margin: 0;"><span style="color: #ff4655;">USER</span><span style="color: #f2f2f2;"> #
    ...
    fetch('https://qr-code-api.oasisx.world/flappy-get-all-user', {
        ...
        .then(data => {
            ...
            let dataRows = data.scores.map((item, index) => {
                return `
                        <p style="margin: 0; color: #ff4655;"><span>${index + 1}</span>#<span>${item.userName}</span></p>
                        <p style="margin: 0%; color: #f2f2f2;">${item.score}</p>
                        <hr style="margin: 15px auto 15px auto; border-color: white; width: 100%;">
                `
            }
            ).join('');
...
function leaderBoard_data(scores=0) {
    ...
    fetch('https://qr-code-api.oasisx.world/flappy-update', {
        ...
        .then(data => {
            ...
            yourScore.innerHTML = `
                <hr style="background: white; border: none; height: 2px;">
                <p id="yourScore"style="color: #f2f2f2; font-size: 25px;"> YOUR SCORE ${score}</p>
                <hr style="background: white; border: none; height: 2px;">
            `;
            yourRank.innerHTML = `
                <span style="color: #f2f2f2; font-size: 20px;">  RANK # ${rank}</span>
                <br>
            `;
        })
        .catch((error) => {
